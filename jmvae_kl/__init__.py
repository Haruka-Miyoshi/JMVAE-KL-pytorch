...
class JMVAEKL(object):
    def __init__(self, x_dim:int, w_dim:int, h_dim:int, z_dim:int, alpha:float=1.0, lr:float=3e-4, train_mode:bool=True, save_path="param", model_path="jmvae_kl_parameter.path"):
    ...
    def KLD2(self, mu0, logvar0, mu1, logvar1):
        loss = -0.5 * torch.sum(1 + logvar0 - logvar1 - ((mu0 - mu1).pow(2) / logvar1.exp()) - (logvar0 - logvar1).exp())
        return loss
    ...
    def train(self, train_loader, epoch=30):
        ...
        for e in range(1, epoch + 1):
            ...
            for i, (batch_data, label) in enumerate(train_loader):
                ...
                mu, logvar, z, xh, wh, mu_x, logvar_x, mu_w, logvar_w = self.model(x, w, True)
                loss = ( self.KLD(mu, logvar) + self.BCE(xh, x) + self.BCE(wh, w) + self.alpha * ( self.KLD2(mu_x, logvar_x, mu, logvar) + self.KLD2(mu_w, logvar_w, mu, logvar) ) ).mean()
                loss.backward()
    ...
    def xw_to_z_to_xhwh(self, x, w):
        ...
        with torch.no_grad():
            mu, logvar, z, xh, wh, mu_x, logvar_x, mu_w, logvar_w = self.model(x, w, False)
