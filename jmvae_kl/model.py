...
class Model(nn.Module):
    def __init__(self, x_dim:int, w_dim:int, h_dim:int, z_dim:int):
        ...
        self.x_to_zx = Encoder(self.x_dim, self.h_dim, self.z_dim)
        self.w_to_zw = Encoder(self.w_dim, self.h_dim, self.z_dim)
    ...
    def forward(self, x, w, mode):
        ...
        mu_x, logvar_x = self.x_to_zx(x)
        zx = self.reparameterize(mu_x, logvar_x, mode)

        mu_w, logvar_w = self.w_to_zw(w)
        zw = self.reparameterize(mu_w, logvar_w, mode)

        xh = self.z_to_x(z)
        wh = self.z_to_w(z)
        return mu, logvar, z, xh, wh, mu_x, logvar_x, mu_w, logvar_w